...
export default function AdminPage() {
  ...
  const analyticsData = useMemo(() => {
    ...
    // Gender distribution
    const genderDistribution = users.reduce((acc: any[], user) => {
    ...
    const qualificationDistribution = users.reduce((acc: any[], user) => {
      const qualification = user.qualification || "Not Specified";
      const existing = acc.find((item) => item.qualification === qualification);
      if (existing) {
        existing.count += 1;
      } else {
        acc.push({ qualification, count: 1 });
      }
      return acc;
    }, []);
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie
                                ...
                                dataKey="count"
                              >
                                {analyticsData.ratingDistribution.map(
                                  (entry, index) => (
                                    <Cell
                                      key={`cell-${index}`}
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie
                                ...
                                dataKey="count"
                              >
                                {analyticsData.genderDistribution.map(
                                  (entry, index) => (
                                    <Cell
                                      key={`cell-${index}`}
                                      fill={
                      ...
                      <div className="bg-gray-50 rounded-lg p-4 sm:p-6">
                        ...
                        <div className="h-64 sm:h-80">
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie
                                data={analyticsData.qualificationDistribution}
                                cx="50%"
                                cy="50%"
                                labelLine={false}
                                ...
                                dataKey="count"
                              >
                                {analyticsData.qualificationDistribution.map(
                                  (entry, index) => (
                                    <Cell
                                      key={`cell-${index}`}
                                      fill={
